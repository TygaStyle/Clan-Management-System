...
$XMLfilename = "http://ak47sforall.wordpress.com/feed.xml";
...
// below.
$allowedTemplateExtensions = Array(".html", ".htm", ".php", ".shtml");
$allowedFeedExtensions = Array(".xml", ".php", ".rss");
...
if ($useFopenURL) {
  ...
  ini_set("user_agent", 'FeedForAll rss2html.php v3');
}
...
if (!isset($_REQUEST["buildURL"])) {
  ...
  if (function_exists("FeedForAll_scripts_readFile")) {
    $_cacheTTL = "";
    if (isset($_POST["XMLCACHTTL"])) {
      $_cacheTTL = $_POST["XMLCACHTTL"];
    }
  ...
  if (function_exists("FeedForAll_scripts_readFile")) {
    ...
    echo "><br>\n";
    echo "Cache Time: <input type=\"text\" name=\"XMLCACHTTL\" size=\"5\" value=\"$_cacheTTL\"> (The number of seconds a file may be cached for before being fetched again)<br>\n";
  }
  ...
  if (isset($_POST["submit"])) {
    ...
    if (function_exists("FeedForAll_scripts_readFile")) {
      ...
      if (isset($_POST["XMLCACHTTL"]) && $_POST["XMLCACHTTL"] != "" && intval($_POST["XMLCACHTTL"] != 0)) {
        $answerAlt .= "\$XMLCACHTTL = \"$_POST[XMLCACHTTL]\";<br>\n";
      }
    }
    ...
    if (isset($_POST["XML"]) && $_POST["XML"] != "") {
      ...
      if ($XMLfilename != "") {
        if (($xmlContents = FeedForAll_scripts_readFile($_POST["XML"], $useFopenURL)) === FALSE) {
          if ($GLOBALS["ERRORSTRING"] == "") {
    ...
    if (isset($_POST["TEMPLATE"]) && $_POST["TEMPLATE"] != "") {
      ...
      if ($TEMPLATEfilename != "") {
        if (($templateContents = FeedForAll_scripts_readFile($_POST["TEMPLATE"], $useFopenURL)) === FALSE) {
          if ($GLOBALS["ERRORSTRING"] == "") {
