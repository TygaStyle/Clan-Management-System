...
if(isset($_GET['db'])) {
	...
	echo "</div><div class=\"center_content\"><div class=\"news_post\">";
		if(!isset($_SESSION['error'])) {
			...
			echo "</tr></td></table><BR>";
			echo "Reference:<BR> - <a href=http://www.EXG-online.fi.st target=_blank>Ask for assistance on the forums</a>";
		}
	echo "</div></div>";
	...
	fclose($handle);
	echo "<td colspan='2'><div align='center'><textarea name='agreeform' id='agreeform' cols='40' rows='5' class=forms>".$contents."</textarea><BR>Agree to Reality Dynasty's Code of Conduct ";
	...
	if(isset($_GET['dm'])) {
		...
		echo "<BR><b>Master Coder:</b> Matt Lo<BR><BR><b>Background Check Program:</b> ViceCaptain<BR><BR><b>Black List Details:</b> KSI Spider 7<BR><BR><b>Website Analyst:</b> KSI Legacy<BR><BR><b>Beta and Delta Tester:</b> KSI Kill Steal<BR><BR><b>Help Section:</b> KSI Elderpooter and KSI Cold 7<BR><BR>Contact KSI ViceCaptain through the forums as a PM (Private Message) or through the chat box.<BR><BR><B>All gamertags are used as identification to hide their real names.</B>";
		} elseif($_GET['dm'] == "5") {
		...
		echo "</table>";
			...
			$find_andrew = mysql_query("SELECT * FROM `mbrlist` WHERE `gametype` = 'Call of Duty 4' AND `visable` = '0'");
...
$end_tab = "</table>"; // ending of table properties of row
	...
	echo "</div><div class=\"center_content\"><div class=\"news_post\">";
		if($_GET['dm'] == "1") {
			if(isset($_GET['view'])) {
				...
				$newrowx = mysql_query("SELECT * FROM `mbrlist` WHERE `visable` = '0'");
				...
				$resulto = mysql_query("SELECT * FROM `divisions` WHERE `visable` = '0' ORDER BY `diviname`");
					while($rowas = mysql_fetch_assoc($resulto)) {
						if($rowas['id'] == 37) {//Block Leader List
							...
							echo "<tr id=ignore><td bgcolor=E0E0E0>".$rowas['diviabbr']." - ".$rowas['diviname']." :: [<a href=index.php?db=clan&dm=1&view=".$rowas['id'].">Division Details</a>]</td></tr>";
			...
			echo "<tr><td>Divisions <i>(RD Based)</i></td><td>".mysql_num_rows($divisions_count)."</td></tr>";
	...
	$aadbresultx = mysql_query("SELECT * FROM `blklist` WHERE `blkname` = '".$_POST['bb']."' LIMIT 0,1");
		...
		echo "<script language=\"javascript\">";
		echo "window.location=\"http://www.EXGCLAN.net/index.php?db=blk&t=".$rowxa['0']."&pz=0&SESSION\";";
		echo "</script>";
		}
	}
	...
	echo "</form>";
		...
		echo "<BR><BR>Level 1- Harrassment, racism, sexism; can be appealed.";
		echo "<BR><BR>Level 2- Cheating in a way that violates the anti-cheating policy; can be appealed.";
		echo "<BR><BR>Level 3- Insubordination, violations of policies; can be appealed.";
		echo "<BR><BR>Level 4- Breaking the Code of Conduct; can be appealed depending on the circumstance.";
		echo "<BR><BR>Level 5- Hitting RD, Hacking or hacking threats, extreme violations of ant of the above; usually cannot be appealed.";
	...
	echo "<tr bgcolor=E0E0E0 id=ignore height=15><td width=130>Username</td><td width=100>Last Active</td><td width=90 align=center><font size=1>Access Level</font></td><td width=150>Access Granted By</td></tr>";
	...
	echo "</div></div>";
		...
		echo "<BR><center><img src=images/blk.jpg></center><BR>";
			if(isset($_GET['t'])) {
				...
				echo "<font size=1><BR><u>Threat Level Details</u>";
				echo "<BR><BR>Level 1- Racism, Sexism; can be appealed.";
				echo "<BR><BR>Level 2- Cheating in a way that violates the anti-cheating policy; can be appealed.";
				echo "<BR><BR>Level 3- Insubordination, violations of policies.";
				echo "<BR><BR>Level 4- Breaking the Code of Conduct; can be appealed depending on the circumstance.";
				echo "<BR><BR>Level 5- Hitting RD, hacking or hacking threats, extreme violations of ant of the above; usually cannot be appealed.";
		...
		if(dataRetrieve($_SESSION['id'], "accesslevel") >= 4) {
			...
			echo "</div><div class=\"center_content\"><div class=\"news_post\">";
			echo "This is where you delete divisions. Level 4 and 5 can delete through RD. <B>ALL ACTIONS ARE RECORDED</B>";
		...
		if(dataRetrieve($_SESSION['id'], "accesslevel") >= 3) {
			...
			echo "Delete Squad";
			echo "</div><div class=\"center_content\"><div class=\"news_post\">";
			echo "This is where you delete squads. Level 3 and up have access to this page. Level 3 can only delete squads within their division. Level 4 and 5 can delete throughout the CMS. <B>ALL ACTIONS ARE RECORDED</B>";
			echo "<form id='formsdsdss' name='formsdsdss' method='post' action='#' onSubmit=\"submitonce(this);\">";
			echo "<table border=0 cellspacing=0 width=95% cellpadding=0 align=center class=tablexx bgcolor=E0E0E0><tr><td><BR>";
				if(dataRetrieve($_SESSION['id'], "accesslevel") == 3) {
		...
		if(dataRetrieve($_SESSION['id'], "accesslevel") >= 3) {
			...
			echo "</div><div class=\"center_content\"><div class=\"news_post\">";
				...
				echo "Add Squad to the database. Level 3 can only add squads to their division. Level 4 and 5 can create throughout the CMS. <b>ALL ACTIONS ARE RECORDED.</b><form id='formsdsd' name='formsdsd' method='post' action='#' onSubmit=\"submitonce(this);\">";
		...
		if(isset($_POST['gensquad'])) {
			...
			$ranvar = rand(1000000000, 9999999999);
			...
			if(isset($_GET['am'])) {
				...
				if($_GET['am'] == "2") {
					if(isset($_GET['s'])) {
						...
						echo "<tr id=ignore><td colspan=3 align=center><a href=index.php?db=adminsettings&am=2>View a User's Log File</a> :: <a href=index.php?db=adminsettings&am=3>View Entire Log File</a> :: <a href=index.php?db=adminsettings&am=4>Empty Log Files</a></td></tr>";
						echo "</table><BR>";
						} else {
						echo "<script language=\"javascript\">";
				...
				echo "Post a message banner across the top of RD's CMS about any breaking news or important information you want the users to view any time.";
...
/*
	...
	//echo "<a href=\"index.php?db=contest\"><img src=\"images/contestad.jpg\" alt=\"RD CMS Contest\" name=\"Image1\" width=\"360\" height=\"100\" border=\"0\" style=\"solid 1px #E0E0E0\"></a><BR>";
	...
	echo "</tr><tr><td valign=top>";
		...
		echo "Total RD Halo: Reach Members: <b>".mysql_num_rows($newrow5)."</b><BR>";
		$newrow6 = mysql_query("SELECT * FROM `mbrlist` WHERE `visable` = '0' AND `gametype` = 'Black Ops'");
		echo "Total RD Black Ops Members: <b>".mysql_num_rows($newrow6)."</b><BR>";
		$newrow61 = mysql_query("SELECT * FROM `mbrlist` WHERE `visable` = '0' AND `gametype` = 'Modern Warfare 2'");
		echo "Total RD Modern Warfare 2 Members: <b>".mysql_num_rows($newrow61)."</b><BR>";
